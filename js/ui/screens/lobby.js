...
class LobbyScreen {
    ...
    bindEvents() {
        ...
        document.querySelectorAll('[data-setting]').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                const currentValue = store.get(`settings.${setting === 'timer' ? 'timerSeconds' : setting === 'categories' ? 'categoriesPerRound' : setting}`);

                const settingKey = setting === 'timer' ? 'timerSeconds' :
                    setting === 'categories' ? 'categoriesPerRound' : 'rounds';
    ...
    renderPlayerList(players) {
        ...
        this.elements.playerList.innerHTML = players.map(player => `
    ...
    updateReadyStatus(players) {
        if (!this.elements.readyStatus) return;
