...
class FinalScreen {
    ...
    renderFinalResults() {
        ...
        if (this.elements.finalStandingsList) {
            this.elements.finalStandingsList.innerHTML = sortedPlayers.map((player, index) => {
                const score = scores[player.id] || 0;
                const position = index + 1;

                let medal = '';
                if (position === 1) medal = '1ST';
                else if (position === 2) medal = '2ND';
                else if (position === 3) medal = '3RD';
                else medal = `${position}TH`;
