...
class AnswerScreen {
    ...
    updateTimer(seconds) {
        if (!this.elements.timerValue || !this.elements.timerDisplay) return;
    ...
    renderAnswerGrid(categories) {
        if (!this.elements.answerGrid || !categories) return;

        const letter = store.get('currentLetter');

        this.elements.answerGrid.innerHTML = categories.map((category, index) => `
